...
- 选中即翻译的读取流程（对齐 TextGO）：① AX `AXSelectedText`（Chromium/Electron 先置 `AXEnhancedUserInterface`/`AXManualAccessibility` 启用其按需建的无障碍树；焦点元素找不到时子元素 BFS 兜底 depth≤6/≤300 节点；短重试 3×60ms 等建树）→ ② 失败回退**模拟 ⌘C + 读剪贴板**（先释放按住的修饰键避免 ⌥⌘C；轮询 ≤300ms；读完**还原**原剪贴板并 `clipboardMonitor.syncChangeCount()`，避免误判"刚复制"；能读到选区范围且为 0 时跳过兜底）
