...
- 调试钩子：`HYMT2_OPEN_SETTINGS / HYMT2_OPEN_HELP / HYMT2_OPEN_CHANGELOG = 1` 启动时自动打开对应窗口；`HYMT2_OPEN_POPOVER=1` 自动弹出菜单栏弹窗（聚焦输入框）；`HYMT2_QUICK_TEXT=<文本>` 模拟"刚复制"触发快捷翻译；`HYMT2_UPDATE_ENGINE=1` 启动即检查引擎更新并自动安装（跳过 12h 节流，端到端验证用）；`HYMT2_UPDATE_APP=1` 同理用于应用自身更新（会替换并重启 App，端到端验证用）；`HYMT2_AX_DEBUG=1` 输出辅助功能权限相关日志（查看：`log show --last 2m --predicate 'eventMessage CONTAINS "[TransendAX]"' --style compact`）
...
- 入口设置最小 `NSApp.mainMenu`（仅「编辑」菜单）：否则弹窗/设置里的文本无法用 ⌘C/⌘V/⌘A（菜单键等价是 AppKit 文本系统的复制粘贴入口）；输入框为 `GrowingTextEditor`（NSTextView 支撑、**自绘占位符**按文本原点对齐，高度随内容 1–5 行、超出滚动），焦点查找需匹配可编辑 `NSTextView`；输出区右上角有复制按钮（复制全部译文）
