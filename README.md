...
**0.1.2**（2026-09-16）
- 修复「选中即翻译」权限问题：开启后启动弹出空权限窗口（`selectToTranslate` 的 didSet 在初始化赋值时也会触发）；未授权 / 授权失效却显示「已授权」（改为必须系统已信任且 AX 探针成功）
- 辅助功能状态改用功能探针（实测未授权返回 -25204/-25208），并结合「是否曾授权过」区分未授权 / 授权失效
- 「一键修复」只弹系统授权提示（不再同时打开设置页），并明确提示需在系统设置手动打开开关；未授权/失效时菜单栏弹窗改用橙色横幅提示，不再启动弹窗打扰
